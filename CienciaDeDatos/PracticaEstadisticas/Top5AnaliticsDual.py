...
st.write(f"<div class='center-text'>Área del polígono de {jugador1_seleccionado}: {area_poligono1:.2f} (Posición {df_jugador1.index[0] + 1}º)</div>", unsafe_allow_html=True)
st.write(f"<div class='center-text'>(Posición {df_jugador1.index[0] + 1}º)</div>", unsafe_allow_html=True)

st.write(f"<div class='center-text'>Área del polígono de {jugador2_seleccionado}: {area_poligono2:.2f} (Posición {df_jugador2.index[0] + 1}º)</div>", unsafe_allow_html=True)
st.write(f"<div class='center-text'>(Posición {df_jugador2.index[0] + 1}º)</div>", unsafe_allow_html=True)


if jugador1_seleccionado == jugador2_seleccionado:
    st.warning("Por favor selecciona dos jugadores diferentes para comparar.")
else:
    # Crear DataFrame para la tabla comparativa
    comparacion = pd.DataFrame(columns=['Estadística', jugador1_seleccionado, jugador2_seleccionado])
    comparacion['Estadística'] = df_jugador1.columns[2:]  # Excluyendo 'Nombre' y 'Equipo'
    comparacion[jugador1_seleccionado] = df_jugador1.iloc[0, 2:].values  # Excluyendo 'Nombre' y 'Equipo'
    comparacion[jugador2_seleccionado] = df_jugador2.iloc[0, 2:].values  # Excluyendo 'Nombre' y 'Equipo'

    # Crear una tabla final sin la posición de los jugadores en los percentiles
    comparacion_final = pd.DataFrame(columns=['Estadística', jugador1_seleccionado, jugador2_seleccionado])

    for estadistica in comparacion['Estadística']:
        comparacion_final = pd.concat([comparacion_final, pd.DataFrame({
            'Estadística': [estadistica],
            jugador1_seleccionado: [comparacion.loc[comparacion['Estadística'] == estadistica, jugador1_seleccionado].values[0]],
            jugador2_seleccionado: [comparacion.loc[comparacion['Estadística'] == estadistica, jugador2_seleccionado].values[0]]
        })], ignore_index=True)
    ...
    with st.expander('Comparación de Estadísticas'):
        st.table(comparacion_final.style.set_table_styles(
            [{'selector': 'thead th', 'props': [('background-color', '#4a4a4a'), ('color', 'white'), ('font-size', '18px'), ('text-align', 'center')]},
            {'selector': 'tbody td', 'props': [('background-color', 'white'), ('color', 'black'), ('font-size', '16px'), ('text-align', 'center')]}]
        ))